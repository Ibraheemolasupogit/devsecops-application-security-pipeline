...
Audience: Security Champions, Software Engineers, Engineering Leads and Product Security. Duration: 60 to 90 minutes. Prerequisites: current developer guidance, relevant evidence reports and a local checkout. Learning objectives cover authentication, authorisation, object access, mass assignment, validation, errors, logging, secrets, dependencies and unsafe subprocess use.

Agenda: introduce scenario, review evidence, identify control expectations, work through the exercise, compare answers, record follow-up actions, and decide whether escalation is needed. Facilitator notes should keep discussion practical and evidence-led. The exercise uses repository artefacts and synthetic examples only. Expected outputs include review notes and requested tests. Assessment criteria are accuracy, evidence quality, risk reasoning, ownership clarity and appropriate escalation. Follow-up actions must reference a command or report.
