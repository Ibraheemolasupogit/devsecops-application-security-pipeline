...
Escalate critical secrets, authentication bypass, authorisation bypass, object-level access bypass, unowned critical or high findings, overdue P1 items, expired exceptions, failed verification, release blocks, repeated vulnerability categories, and tooling failures affecting assurance.

```mermaid
flowchart TD
  Squad[Squad handling] --> Champion[Champion escalation]
  Champion --> Lead[Engineering Lead escalation]
  Lead --> PS[Product Security escalation]
  PS --> Risk[Risk Owner escalation]
```

Evidence must include the command output or report, canonical finding or lifecycle record, affected control, owner, due date, release impact, and proposed action.
